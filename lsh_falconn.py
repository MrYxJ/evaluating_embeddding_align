...
def generate_candidate_threshold(entity_embedding=None, data_ids="OpenEA", path="", threshold=0.2, output_path=False,
                                 entity_file="ent_embeds,npy", normalize=True, metric="inner", lsh_family="crosspolytope"):
    """
    ...
    # kg2_data = np.array([entity_embedding[e] for e in data_ids[:, 1]])
    ...
    for pair in data_ids:
        ...
        ans = query_object.find_near_neighbors(R_entity, threshold=threshold)
...
def generate_candidate_top(entity_embedding=None, data_ids="OpenEA", path="", top_k=5, output_path=False, entity_file="ent_embeds,npy",
                           normalize=True, metric="inner", lsh_family="crosspolytope"):
    """
    :param entity_embedding:
    :param data_ids:
    :param path:
    ...
    if data_ids is "OpenEA":
        ent2id1, id2ent1 = read_ent_id(path + "kg1_ent_ids")
        ent2id2, id2ent2 = read_ent_id(path + "kg2_ent_ids")
    ...
    if os.path.exists(path + "mapping_mat.npy"):  # OpenEA的规定
        ...
        kg1_data = np.matmul(kg1_data, mapping)
        print("load mapping  succussuflly!")
    #kg2_dataset = entity_embedding[data_ids[:, 1]]
    kg1_index = data_ids[:, 0].tolist()
    number_of_tables = 500
    params_cp = falconn.LSHConstructionParameters()
    params_cp.dimension = entity_embedding.shape[1]
...
def generate_hits(entity_embedding=None, data_ids=None, path="", entity_file="ent_embeds,npy", metric = "cosine", hits=(1,5,10), normalize=True):
    print("path:", path)
    if entity_embedding is None:
        entity_file_path = path + entity_file
        entity_embedding = np.load(entity_file_path)
...
def static_openea_mode(path="/home/kgbnu/Code/output/results/", fold="/721_5fold/1/", model="MTransE",
                       threshold="", top_k=0, hits=(), metric="inner", csls=0, normalize=True, data_name=None):
    """
    实现lshg
...
if __name__ == '__main__':
    ...
    for model in models:
        static_openea_mode(model=model, hits=(1, 5, 10), metric=metric)
        print("\n")

    for threshold in thresholds:
        for model in models:
            static_openea_mode(model=model, threshold=threshold, metric=metric)
            print("\n")
    
    for topk in top_ks:
        for model in models:
            static_openea_mode(model=model, top_k=topk, metric=metric)
            print("\n")
